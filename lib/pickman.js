...
<el-col class="content list">
  ...
  <el-button type="primary" class="jumpBudgetSet" @click="">新建按钮</el-button>
  <el-table
      :data="tableData"
      border
      style="width: 100%">
          ...
          <el-table-column
              ...
              width="200">
            <span>
              <el-button type="button" size="small"  @click="seeDetail(row.id)">查看</el-button>
            <template v-if="row.state == 3">
              <el-button type="button" size="small"  @click="edit(row.id)">编辑</el-button>
            </template>
            <template v-if="row.state == 0">
              <el-button type="button" size="small"  @click="goBack(row.id)">撤回</el-button>
            </template>
            </span>
          </el-table-column>
  </el-table>
    ...
    init: function(res) {
        ...
        new Vue({
            ...
            methods: {
                ...
                /**
                 * @function
                 * @description 跳转到一个新的页面
                 */
                seeDetail(id) {
                    window.open('/budget/mis/showapply?apply_id=' + `${id}` + "&is_show_log=1");
                }
            }
        });
    }
